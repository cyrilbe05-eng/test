...
### 1.2 Choose dev or production mode

The portal uses a fully custom login UI — it never shows Clerk's hosted pages. This means:

- **Development mode** (`pk_test_` / `sk_test_` keys) works fine for production use. There is no Clerk branding visible to users.
- **Production mode** (`pk_live_` / `sk_live_` keys) is optional but recommended if you want Clerk's full uptime SLA and email deliverability guarantees.

> **Free plan note:** Switching to production mode requires a paid Clerk plan. If you stay on the free tier, just use the development keys — the app will work identically. The only difference is internal to Clerk's infrastructure.
...
### 1.4 (Optional) Enable the email allowlist

This prevents anyone not on the list from signing in.

1. Clerk dashboard → **Configure → Restrictions**
2. Turn on **Allowlist**
3. Add emails one by one as you create users (or skip this — user creation is admin-only anyway)
...
The project has five schema files that must all be applied, in order. Run each one separately.
...
### 3.1 Create the account and choose a plan
...
| Plan | Cost | Works? |
|---|---|---|
| Hobby (free) | $0 | No — limited to 12 serverless functions; this project has ~55 API routes |
| Pro | $20/mo | Yes — unlimited functions |

> Upgrade to **Pro** before deploying.
...
### 4.2 (If allowlist is enabled) Add to allowlist
...
- [ ] All 5 schema files + seed.sql were executed successfully in D1

...
export function useStorageAdapter(): StorageAdapter {
  ...
  return {
    ...
    async getSignedUrl(key, _expiresInSeconds = 3600) {
      // key is the storage_key; we need to find the file id first
      // For the signed-url endpoint we use the file id, not the storage key.
      // This method is kept for backwards compatibility but callers should
      // prefer getSignedUrlById when they have the file id.
      // Here we use the key as an encoded path — the API route looks up by id.
      // Fall back: the key itself is used as the id param here when no id is available.
      const { signedUrl } = await apiFetch<{ signedUrl: string }>(
        `/api/project-files/${encodeURIComponent(key)}/signed-url`
      )
      return signedUrl
    },

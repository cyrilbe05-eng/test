...
import { MOCK_PROJECTS, MOCK_PROFILES, formatProjectTitle } from '../mockData'
...
// ── Nav links ──────────────────────────────────────────────────────────────────
const NAV_LINKS = [
  { to: '/admin',           label: 'Projects',  icon: <IconLayout /> },
...
export default function DemoAdminProjects() {
  ...
  const getClientName = (clientId: string) =>
    MOCK_PROFILES.find((p) => p.id === clientId)?.full_name ?? '—'
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden" style={{ minHeight: 'calc(100vh - 52px)' }}>
        ...
        >
          ...
          <div className="px-3 pb-4 whitespace-nowrap">
            <p className="text-[10px] text-muted-foreground truncate">{projects.length} projects total</p>
          </div>
        </aside>
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6">
          ...
          <div className="flex items-center justify-between mb-6 animate-slide-up">
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-0.5">{projects.length} total</p>
            </div>
          ...
          {/* Views */}
          {view === 'kanban' ? (
            <DemoKanban projects={projects} getClientName={getClientName} onDrop={handleDrop} />
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left bg-muted/20">
                    {['Client', 'Title', 'Status', 'Created', 'Updated'].map((h) => (
                ...
                <tbody className="divide-y divide-border/50">
                  {projects.map((p, i) => (
                    <tr
                      key={p.id}
                      className={cn('hover:bg-muted/20 transition-colors animate-slide-up', `stagger-${Math.min(i + 1, 7)}`)}
                    >
                      <td className="px-4 py-3 text-muted-foreground text-sm">{getClientName(p.client_id)}</td>
                      <td className="px-4 py-3">
                        <Link to={`/admin/projects/${p.id}`} className="text-primary hover:underline font-medium">
      ...
      {profile && <ChatPanel currentUserId={profile.id} isAdmin />}
    </div>
  )
}
      ...
      {COLUMNS.map((col, colIdx) => {
        ...
        return (
          <div
            ...
            onDrop={() => {
              if (dragging && dragging.from !== col.status) onDrop(dragging.id, dragging.from, col.status)
              setDragging(null)
              setDragOver(null)
            }}
          >
            ...
            <div className="flex-1 px-2 pb-3 space-y-2 min-h-[60px]">
              {cards.map((p, i) => (
                <div
                  key={p.id}
                  draggable
                  onDragStart={() => setDragging({ id: p.id, from: p.status })}
                  onDragEnd={() => { setDragging(null); setDragOver(null) }}
                  className={cn(
                    'clay-card p-3 cursor-grab active:cursor-grabbing transition-all duration-150 animate-slide-up hover:shadow-lg hover:-translate-y-0.5',
                    dragging?.id === p.id && 'opacity-40 scale-95',
                    `stagger-${Math.min(i + 1, 7)}`,
                  )}
                >
                  <Link to={`/admin/projects/${p.id}`} onClick={(e) => e.stopPropagation()} className="block">
